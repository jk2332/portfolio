...
#define PINCH_CLOUD_DIST_OFFSET     6
...
void GameScene::combineByPinch(Cloud* cind1, Cloud* cind2, Vec2 pinchPos){
    ...
    if (x_left_gap >= -PINCH_OFFSET && x_right_gap >= -PINCH_OFFSET && y_bottom_gap >= -PINCH_OFFSET && y_top_gap >= -PINCH_OFFSET){
        ...
        cind2->incSize(cind1->getCloudSize() - 1.0f);
        long toDelete = -1;
...
//    CULog("updating in gamecontroller");
    _input.update(dt);
    ticks++;
    ...
    for (auto const& touchID : IDs) {
        if (selected.count(touchID)){
            ...
            if (!_selectors.count(touchID)){
                ...
                selector->setDebugColor(Color4(255, 0, 0));
...
//            Vec2 ori_size = c->getSize();
//            shape->SetAsBox(ori_size.x*c->getCloudSize()/64, ori_size.y*c->getCloudSize()/64);
//            c->cugl::PolygonObstacle::setSize(ori_size.x*c->getCloudSize(), ori_size.y*c->getCloudSize());
//            cloudNode->setContentSize(cloudNode->getContentSize()*c->getCloudSize());
...
void GameScene::splitClouds(){
    ...
    for (auto &ic : cloudsToSplit){
        ...
        cloud->setCloudScale(_scale);
        cloud->setCloudSize(c->getCloudSize());
