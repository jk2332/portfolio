...
void Plant::setSceneNode(const std::shared_ptr<cugl::Node>& node, std::string name){
    ...
    cugl::Vec2 b = cugl::Vec2((0.75f + DOWN_LEFT_CORNER_X + GRID_WIDTH*_x + GRID_WIDTH/2)*32.0f, (0.3f + DOWN_LEFT_CORNER_Y + GRID_HEIGHT*_y - GRID_HEIGHT/2)*32.0f);
    _signNode->setPosition(b);

    _signIcon = PolygonNode::allocWithTexture(_assets->get<Texture>("iconHappy"));
    _signIcon->setScale(0.2f);
    cugl::Vec2 c = cugl::Vec2((0.75f + DOWN_LEFT_CORNER_X + GRID_WIDTH*_x + GRID_WIDTH/2)*32.0f, (0.3f + DOWN_LEFT_CORNER_Y + GRID_HEIGHT*_y - GRID_HEIGHT/2)*32.0f);
